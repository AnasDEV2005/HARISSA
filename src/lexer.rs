...
pub fn tokenize(contents: String) -> Vec<Token> {
    ...
    loop {
        ...
        if c == None {
            tokens.push(Token::EndOfFile(linecount - 2));
            break;
        } // END OF FILE
          // HACK: idk why it
        ...
        match character {
            ...
            '|' | '&' | '=' => match character {
                '|' => {
                    ...
                    match next {
                        ...
                        Some('|') => {
                            tokens.push(Token::DblOrBool(linecount));
                        }
                        _ => {
                            tokens.push(Token::InvalidToken((character.to_string(), linecount)));
                        }
                ...
                '&' => {
                    ...
                    match next {
                        ...
                        Some('&') => {
                            tokens.push(Token::DblAndBool(linecount));
                        }
                        _ => {
                            tokens.push(Token::InvalidToken((character.to_string(), linecount)));
                        }
                ...
                '=' => {
                    ...
                    match next {
                        ...
                        Some('=') => {
                            tokens.push(Token::BoolEqual(linecount));
                        }
                        _ => {
                            tokens.push(Token::InvalidToken((character.to_string(), linecount)));
                        }
            ...
            '{' | '}' => {
                ...
                if character == '{' {
                    tokens.push(Token::OpenCurlyBracket(linecount));
                } else {
                    tokens.push(Token::CloseCurlyBracket(linecount));
                }
            }
            ...
            '(' | ')' => {
                ...
                if character == '(' {
                    tokens.push(Token::OpenParenth(linecount));
                } else {
                    tokens.push(Token::CloseParenth(linecount));
                }
            }
            ...
            '[' | ']' => {
                ...
                if character == '[' {
                    tokens.push(Token::OpenSquareBracket(linecount));
                } else {
                    tokens.push(Token::CloseSquareBracket(linecount));
                }
            }
                ...
                match character {
                    '-' => {
                        ...
                        match next {
                            None => {
                                tokens
                                    .push(Token::InvalidToken((character.to_string(), linecount)));
                                break;
                            }
                            ...
                            Some('=') => {
                                tokens.push(Token::AssignDecrement(linecount));
                            }
                            _ => {
                                tokens
                                    .push(Token::InvalidToken((character.to_string(), linecount)));
                    ...
                    '+' => {
                        ...
                        */
                        ...
                        match next {
                            ...
                            Some('=') => {
                                tokens.push(Token::AssignIncrement(linecount));
                            }
                ...
                if keyword_or_ident.is_empty() {
                    ...
                    if ch.is_digit(10) {
                        ...
                        loop {
                            ...
                            if !next_image
                                .expect("Unwrapping ERROR lexer.rs at _ arm")
                                .is_digit(10)
                            {
                                tokens
                                    .push(Token::InvalidToken((character.to_string(), linecount)));
...
fn unwrap_word(word: &String, linecount: i32) -> Token {
    match word.as_str() {
        "const" | "while" | "for" | "loop" | "if" | "else" | "fn" | "pub" | "string" | "int"
        | "uint" | "object" | "list" | "variant" | "tuple" | "ERROR_RULES" => {
            Token::Keyword((word.to_string(), linecount))
        } // i might make a token for every keyword idk
